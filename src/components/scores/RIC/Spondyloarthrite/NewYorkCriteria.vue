<template>
  <section>
    <h1 class="title">critères de New-York modifié</h1>

    <form class="form">
      <h3 class="">Critère cliniques</h3>

      <label for="lombagies"
        >Lombagies avec raideur de plus de trois mois, améliorées à l'éffort,
        mais ne cédant pas au repos.</label
      >
      ...
      <label for="rachis"
        ...
        frontal et sagittal</label
      >
      <input id="rachis" type="checkbox" v-on:click="changeCriterionScore" />
      <br />
      <label for="thoracique"
        >Limitation de l'amplition thoracique par rapport aux valeurs normales
        corrigés pour l'âge et le sexe</label
      >
      <input
        id="thoracique"
        type="checkbox"
        v-on:click="changeCriterionScore"
      />
      ...
      <input
        id="unilatérale"
        type="checkbox"
        v-on:click="changRadiologyScore"
      />
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      totalRadiology: 0,
      result: false,
      // permet d'afficher le résultat au moment du clique
      toggle: false,
    };
  },
  methods: {
    ...
    showResult() {
      this.totalCriterion >= 1 && this.totalRadiology >= 1
        ? ((this.result = true), (this.toggle = true))
        : ((this.result = false), (this.toggle = true));
    },
